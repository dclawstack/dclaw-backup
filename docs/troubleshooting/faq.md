...
### How do I update DClaw Backup?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp backup --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Backup without Kubernetes?

Yes, for local development:

```bash
cd dclaw-backup/frontend && npm run dev
cd dclaw-backup/backend && uvicorn dclaw_backup.main:app --reload
```
